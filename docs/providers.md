...
- Player-facing runs: mix DeepSeek official and SudoCode's Codex-format endpoint.
- Do not use SudoCode's Claude Code-format endpoint.
- TKen Grok is a usable reserve provider. Keep Lumin Grok out of the active pool until its routing and fixed context are understood.
...
| TKen Grok                   | Responses, requested and returned `grok-4.6`               | Baseline input was 226 tokens, including 192 cached tokens. With political `instructions`, input was 292 tokens and the model returned the requested JSON. Reasoning was present.                        | Usable reserve provider.                                                                    |
