...
For a real two-decision short game, run `pnpm dev`, open the local page, and choose **DeepSeek / 真实 NPC**. The local service reads `DEEPSEEK_API_KEY` or the single key in `secret/deepseek.txt`. Starting a scene makes no model call; each decree calls the official model with thinking enabled. The commander keeps his prior experiences, while the ruler sees a timeline of received reports and their own orders. Errors pause the current decision and offer an explicit retry. Completed records stay under gitignored `runs/`; whole-game review and replay do not make model calls. Unfinished games are in memory and are lost on server restart. This route requires the local development server, not just the static build.
